...
public:
    ...
    [[nodiscard]] auto get_nodes() const noexcept 
        -> std::span<const std::unique_ptr<Node>>;
    ...
    [[nodiscard]] auto get_connections() const noexcept 
        -> std::span<const Connection>;
    
    /// @brief Get connections from a node
    [[nodiscard]] auto get_connections_from(NodeId node) const 
        -> std::vector<ConnectionId>;
    
    /// @brief Get connections to a node
    [[nodiscard]] auto get_connections_to(NodeId node) const 
        -> std::vector<ConnectionId>;
    ...
    [[nodiscard]] auto find_reachable_nodes(NodeId start) const 
    ...
    [[nodiscard]] auto validate_connection(
        NodeId from_node,
        PortId from_port,
        NodeId to_node,
        PortId to_port
    ...
    [[nodiscard]] auto get_nodes_of_type(NodeType type) const 
