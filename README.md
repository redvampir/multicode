...
| UE C++ | ✅ Готово | Отдельный target генерации, UE-макросы и class pipeline |
| Rust | ⚠️ Не поддерживается | Язык можно выбрать, но генератор даёт явное сообщение о неподдержке |
| ASM | ⚠️ Не поддерживается | Язык можно выбрать, но генератор даёт явное сообщение о неподдержке |
...
- **Внутренние вехи roadmap:** v0.1–v0.5 завершены; v0.6 реализован end-to-end, текущий фокус — стабилизация и UX
...
- **Генерация кода** для `cpp` и `ue` из визуального графа
- Локализация RU/EN с мгновенным переключением
- Undo/Redo, Copy/Paste, автолейаут, контекстное меню
- Система пакетов узлов
- Пользовательские функции: UI, модель и кодогенерация работают end-to-end
- Dependency View v2 и Class System v2 присутствуют в рабочем UX
...
- Кодогенерация пользовательских функций (C++) работает end-to-end; дальше полировка UX и стабилизация (см. `ROADMAP.md`)
