...
auto Graph::remove_node(NodeId id) -> Result<void> {
    ...
    auto it = std::ranges::find_if(nodes_, [id](const auto& node) {
...
auto Graph::connect(
    ...
    // Validate connection
    if (auto result = validate_connection(from_node, from_port, to_node, to_port); 
        !result) {
    ...
    const auto conn_type = from_port_ptr->is_execution() 
...
auto Graph::topological_sort() const -> Result<std::vector<NodeId>> {
    ...
    for (const auto& node : nodes_) {
        ...
        if (!visited.contains(node_id)) {
            const bool has_cycle = topological_sort_dfs(
                node_id, 
                visited, 
                in_stack, 
                result
...
auto Graph::topological_sort_dfs(
    NodeId node,
    std::unordered_set<NodeId>& visited,
    std::unordered_set<NodeId>& in_stack,
    std::vector<NodeId>& result
...
auto Graph::find_strongly_connected_components() const 
...
auto Graph::validate() const -> ValidationResult {
    ...
    if (start && !connections_.empty()) {
        ...
        for (const auto& node : nodes_) {
            if (!reachable.contains(node->get_id()) && 
    ...
    for (const auto& conn : connections_) {
        if (auto conn_result = validate_connection(
...
auto Graph::validate_connection(
    NodeId from_node,
    PortId from_port,
    NodeId to_node,
    PortId to_port
...
auto Graph::find_nodes_by_name(std::string_view pattern) const 
...
auto Graph::get_metadata(std::string_view key) const 
...
auto Graph::remove_node_connections(NodeId node) -> void {
    ...
    // Remove duplicates
    std::ranges::sort(to_remove, [](ConnectionId a, ConnectionId b) {
...
auto Graph::validate_port_exists(NodeId node, PortId port) const -> Result<void> {
    ...
    if (!port_ptr) {
        return Result<void>(Error{
            .message = std::format("Port {} does not exist on node {}", 
                                  port.value, node.value),
