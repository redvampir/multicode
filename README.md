...
- Отдельно проверяйте `include/visprog/core/Port.hpp` и сигнатуры соединений в `Graph.hpp`: CI жёстко валится на `-Wclang-format-violations`, если руками поправить отступы или перенести аргументы без `clang-format`.
