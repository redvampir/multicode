...
auto Node::add_input_port(DataType data_type, std::string name) -> PortId {
    ...
    ports_.emplace_back(port_id, PortDirection::Input, data_type, std::move(name));
...
auto Node::add_output_port(DataType data_type, std::string name) -> PortId {
    ...
    ports_.emplace_back(port_id, PortDirection::Output, data_type, std::move(name));
...
auto Node::add_exec_input() -> PortId {
    ...
    ports_.emplace_back(port_id, PortDirection::Input, DataType::Execution, "exec_in");
...
auto Node::add_exec_output() -> PortId {
    ...
    ports_.emplace_back(port_id, PortDirection::Output, DataType::Execution, "exec_out");
...
auto Node::validate() const -> Result<void> {
    ...
    if (name_.empty()) {
        return Result<void>(Error{.message = "Node name cannot be empty", .code = 100});
    }
    ...
    if (has_execution_flow_) {
        ...
        if (exec_inputs.empty() && exec_outputs.empty()) {
            return Result<void>(
                Error{.message = "Node marked as having execution flow but has no exec ports",
                      .code = 101});
        }
    }
    ...
    std::ranges::sort(port_ids, [](PortId a, PortId b) { return a.value < b.value; });

    auto [first, last] = std::ranges::unique(port_ids);
    if (first != last) {
        return Result<void>(Error{.message = "Duplicate port IDs found", .code = 102});
    }
    ...
    switch (type_) {
        ...
        case NodeType::End:
            ...
            if (get_exec_input_ports().empty()) {
                return Result<void>(Error{
                    .message = "End node must have at least one execution input", .code = 106});
            }
            break;
        ...
        case NodeType::PureFunction:
            ...
            if (has_execution_flow_) {
                return Result<void>(Error{
                    .message = "Pure function nodes cannot have execution flow", .code = 107});
            }
            break;
