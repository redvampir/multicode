...
[[nodiscard]] constexpr auto port_direction_to_string(PortDirection direction) noexcept -> std::string_view {
    switch (direction) {
...
[[nodiscard]] constexpr auto connection_type_to_string(ConnectionType type) noexcept -> std::string_view {
    switch (type) {
...
[[nodiscard]] auto parse_metadata(const nlohmann::json& object,
    ...
    if (!object.is_object()) {
        return Result<void>(Error{
            .message = std::string(context) + ": metadata должно быть объектом",
    ...
    for (const auto& [key, value] : object.items()) {
        if (!value.is_string()) {
            return Result<void>(Error{
                .message = std::string(context) + ": значение metadata '" + key + "' должно быть строкой",
...
[[nodiscard]] auto validate_schema(const nlohmann::json& document) -> Result<void> {
    ...
    if (schema_it == document.end() || !schema_it->is_object()) {
        return Result<void>(Error{
            .message = "Отсутствует объект 'schema'",
    ...
    if (version.value() != GraphSerializer::kSchemaVersion) {
        return Result<void>(Error{
            .message = std::format(
                "Несовместимая версия схемы '{}', поддерживается '{}'",
                version.value(),
    ...
    if (core_min.value() != GraphSerializer::kSchemaCoreMin ||
        ...
        return Result<void>(Error{
            .message = std::format(
                "Несовместимое окно версий ядра [{} - {}]",
                core_min.value(),
                core_max.value()
            ),
            .code = kErrorSchemaVersion
    ...
    if (kCoreVersion < GraphSerializer::kSchemaCoreMin || kCoreVersion > GraphSerializer::kSchemaCoreMax) {
        return Result<void>(Error{
            .message = std::format(
                "Текущая версия ядра '{}' не входит в окно [{} - {}]",
                kCoreVersion,
...
auto GraphSerializer::to_json(const Graph& graph) -> nlohmann::json {
    ...
    document["schema"] = {
        {"version", GraphSerializer::kSchemaVersion},
    ...
    for (const auto& connection : graph.get_connections()) {
        ...
        connection_json["from"] = {
            {"nodeId", connection.from_node.value},
            {"portId", connection.from_port.value}
        };
        connection_json["to"] = {
            {"nodeId", connection.to_node.value},
...
auto GraphSerializer::from_json(const nlohmann::json& document) -> Result<Graph> {
    if (!document.is_object()) {
        return Result<Graph>(Error{
            .message = "Root JSON должен быть объектом",
            .code = kErrorInvalidDocument
        });
    }
    ...
    if (graph_it == document.end() || !graph_it->is_object()) {
        return Result<Graph>(Error{
            .message = "Отсутствует объект 'graph'",
    ...
    if (nodes_it == document.end() || !nodes_it->is_array()) {
        return Result<Graph>(Error{
            .message = "Ожидается массив 'nodes'",
            .code = kErrorMissingField
        });
    }
    ...
    for (std::size_t index = 0; index < nodes_it->size(); ++index) {
        ...
        if (!node_json.is_object()) {
            return Result<Graph>(Error{
                .message = std::format("nodes[{}] должен быть объектом", index),
                .code = kErrorInvalidDocument
            });
        }

        const auto node_id_result = require_uint64(node_json, "id", std::format("nodes[{}]", index), kErrorMissingField);
        if (!node_id_result) {
            return Result<Graph>(node_id_result.error());
        }
        const NodeId node_id{node_id_result.value()};

        const auto type_result = require_string(node_json, "type", std::format("nodes[{}]", index), kErrorMissingField);
        if (!type_result) {
        ...
        const auto name_result = require_string(node_json, "name", std::format("nodes[{}]", index), kErrorMissingField);
        if (!name_result) {
        ...
        if (auto display_it = node_json.find("displayName"); display_it != node_json.end() && display_it->is_string()) {
            node->set_display_name(display_it->get<std::string>());
        }

        if (auto desc_it = node_json.find("description"); desc_it != node_json.end() && desc_it->is_string()) {
            node->set_description(desc_it->get<std::string>());
        }

        if (auto metadata_it = node_json.find("metadata"); metadata_it != node_json.end()) {
            if (auto meta_result = parse_node_metadata(*metadata_it, std::format("nodes[{}]", index), *node); !meta_result) {
                return Result<Graph>(meta_result.error());
            }
        }
        ...
        if (ports_it == node_json.end() || !ports_it->is_array()) {
            return Result<Graph>(Error{
                .message = std::format("nodes[{}]: отсутствует массив 'ports'", index),
                .code = kErrorMissingField
            });
        }
        ...
        for (std::size_t port_index = 0; port_index < ports_it->size(); ++port_index) {
            ...
            if (!port_json.is_object()) {
                return Result<Graph>(Error{
                    .message = std::format("nodes[{}].ports[{}] должен быть объектом", index, port_index),
                    .code = kErrorInvalidDocument
                });
            }

            const auto port_id_result = require_uint64(port_json, "id",
            ...
            const auto dir_result = require_string(port_json, "direction",
                                                   std::format("nodes[{}].ports[{}]", index, port_index),
                                                   kErrorMissingField);
            if (!dir_result) {
            ...
            if (!direction.has_value()) {
                return Result<Graph>(Error{
                    .message = std::format(
                        "nodes[{}].ports[{}]: неизвестное направление '{}'",
                        index,
                        port_index,
                        dir_result.value()
                    ),
            ...
            const auto data_type_result = require_string(port_json, "dataType",
                                                         std::format("nodes[{}].ports[{}]", index, port_index),
                                                         kErrorMissingField);
            if (!data_type_result) {
            ...
            if (!data_type.has_value()) {
                return Result<Graph>(Error{
                    .message = std::format(
                        "nodes[{}].ports[{}]: неизвестный тип данных '{}'",
                        index,
                        port_index,
            ...
            const auto port_name_result = require_string(port_json, "name",
                                                          std::format("nodes[{}].ports[{}]", index, port_index),
                                                          kErrorMissingField);
            if (!port_name_result) {
            ...
            Port port(PortId{port_id_result.value()}, direction.value(), data_type.value(), port_name_result.value());
            if (auto type_name_it = port_json.find("typeName"); type_name_it != port_json.end()) {
                if (!type_name_it->is_string()) {
                    return Result<Graph>(Error{
                        .message = std::format(
                            "nodes[{}].ports[{}]: 'typeName' должен быть строкой",
                            index,
                            port_index
                        ),
                        .code = kErrorMetadataValue
        ...
        if (!added_id || added_id != node_id) {
            return Result<Graph>(Error{
                .message = std::format("Не удалось добавить узел nodes[{}]", index),
                .code = kErrorInvalidDocument
            });
        }
    ...
    if (connections_it != document.end()) {
        if (!connections_it->is_array()) {
            return Result<Graph>(Error{
                .message = "'connections' должен быть массивом",
                .code = kErrorInvalidDocument
            });
        }
        ...
        for (std::size_t index = 0; index < connections_it->size(); ++index) {
            ...
            if (!conn_json.is_object()) {
                return Result<Graph>(Error{
                    .message = std::format("connections[{}] должен быть объектом", index),
                    .code = kErrorInvalidDocument
                });
            }

            const auto conn_id_result = require_uint64(conn_json, "id",
                                                       std::format("connections[{}]", index),
                                                       kErrorMissingField);
            if (!conn_id_result) {
                return Result<Graph>(conn_id_result.error());
            }

            const auto type_result = require_string(conn_json, "type",
                                                    std::format("connections[{}]", index),
                                                    kErrorMissingField);
            if (!type_result) {
                return Result<Graph>(type_result.error());
            ...
            if (from_it == conn_json.end() || to_it == conn_json.end() ||
                !from_it->is_object() || !to_it->is_object()) {
                return Result<Graph>(Error{
                    .message = std::format("connections[{}]: поля 'from' и 'to' обязательны", index),
                    .code = kErrorMissingField
                });
            }

            const auto from_node_result = require_uint64(*from_it, "nodeId",
                                                          std::format("connections[{}].from", index),
                                                          kErrorMissingField);
            if (!from_node_result) {
                return Result<Graph>(from_node_result.error());
            }
            const auto from_port_result = require_uint64(*from_it, "portId",
                                                          std::format("connections[{}].from", index),
                                                          kErrorMissingField);
            if (!from_port_result) {
            ...
            const auto to_node_result = require_uint64(*to_it, "nodeId",
                                                        std::format("connections[{}].to", index),
                                                        kErrorMissingField);
            if (!to_node_result) {
                return Result<Graph>(to_node_result.error());
            }
            const auto to_port_result = require_uint64(*to_it, "portId",
                                                        std::format("connections[{}].to", index),
                                                        kErrorMissingField);
            if (!to_port_result) {
            ...
            Connection connection{
                .id = ConnectionId{conn_id_result.value()},
