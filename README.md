...
1. **Сериализация графа** (JSON через nlohmann): чтение/запись узлов, портов, связей и метаданных.
2. **Минимальный VS Code UI**: webview с отрисовкой через Cytoscape, загрузка/сохранение JSON, вызовы core через wasm/ffi или RPC.
3. **Генератор C++**: обход графа в топологическом порядке, сопоставление узлов шаблонам кода, базовая поддержка переменных и ветвлений.
4. **Интеграция clang-tidy/clang-format в CI**: чтобы ловить регрессии раньше.
